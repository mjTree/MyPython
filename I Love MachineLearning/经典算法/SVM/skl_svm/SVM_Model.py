...
return: 输入的参数值
"""
def checkParam(filename, split, function):
    if filename == '':
        print('数据集不存在,请重新读取！！')
        sys.exit()
    if str(int(float(split))).isdigit() and (float(split)>0 and float(split)<1):
        ...
        split = '0.7'  #默认分割0.7
        split = float(split)
    if str(function).isdigit():
        function = int(function)
    else:
        function = '1'     #默认为4
        function = int(function)
    return filename,split,function
...
parameter: function 核函数
return: clf 通过训练集得到的分类器
"""
def svmClassification(dataSet,labelSet,function):
    if function == 1:
        clf = svm.SVC(kernel = 'poly', degree=3)    # 多项式核函数
    elif function == 2:
        clf = svm.SVC(kernel = 'linear')            # 线性核函数
    elif function == 3:
        clf = svm.SVC(kernel = 'rbf', gamma=0.7)    # 径向基核函数
